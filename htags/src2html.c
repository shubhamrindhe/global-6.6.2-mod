...
put_brace(const char *text)
{
	...
	char* divOpen1 = "<div class='codeBlock' id='";
	char* divOpen2 = "d'>";
	char* divClose= "</div>";
	
	char* checkbox1 = "<input type='checkbox' id='";
	char* checkbox2 = "'checked='checked'/><label onmousemove='setTooltip(this)' onmouseout='clearTooltip(this)' for='";
	char* checkbox3 = "'class='blockLabel'></label>";
	int i = 0;
	
	if(*text=='{'){

		strbuf_puts(outbuf, checkbox1);
		strbuf_puts(outbuf, ID);
		strbuf_puts(outbuf, checkbox2);
		strbuf_puts(outbuf, ID);	
		strbuf_puts(outbuf, checkbox3);
		++id;
		

		strbuf_puts(outbuf, divOpen1);
		strbuf_puts(outbuf, ID);
		strbuf_puts(outbuf, divOpen2);


		int i = 0;
		for(i=0;i<indent;++i){
			strbuf_puts(outbuf, tab);
		}
